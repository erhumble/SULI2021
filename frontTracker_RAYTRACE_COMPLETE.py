...
TRACK_raytracing = True
TRACK_midpointGradient = False
save_and_plot_blobs = False
save_and_plot_tracks = False
PLOT_ALL_TRACKERS = True
...
for filename in filename_list:
    ...
    save_path = open_path + 'raytrace/'
    if not os.path.exists(save_path):
            os.makedirs(save_path)
    
    ########### adding Rupak's functions ##################################
    seed(99999999)
    ...
    thresholdDensity = n0*1.5

    ############################################
    data_num = np.arange(start=timeframe_start, stop=timeframe_end, step=dt, dtype=int)
    f = ad.file(open_path  + '%s_electron_M0_%d.bp' %(filename, data_num[0]))

    blob_size_file = open(save_path + "blob_size.txt", "w")
    ...
    def func_data(ionDensityData,phiData):
            ...
            interpGrid_M0, ionDensityValues = ionDensityInterp.interpolate()
            interpGrid_phi, phiValues = phiInterp.interpolate()
            ...
            CCC = []
            for j in range(0,len(interpGrid_M0)):
                CCC.append((interpGrid_M0[j][1:] + interpGrid_M0[j][:-1])/2)
            x_vals = CCC[0]
            y_vals = CCC[1]
            z_vals = CCC[2]
    ...
    def animate(i):
            blob_counter = 0
            ionDensity= open_path + '%s_electron_M0_%d.bp' %(filename, data_num[i])
            phi= open_path + '%s_phi_%d.bp' %(filename, data_num[i])
            ...
            for j in range(contour_number):
                ...
                if (abs(x[0]-x[len(x)-1]) <= 1e-10) and blobLimX > 2*dx and blobLimY > 2*dy:
                    ...
                    if (confidence > 0.80):
                        ...
                        if imageCounter == 0:
                            plt.savefig(save_path + "file_number%d"%data_num[i]+"_blob_snap.png")   # save the figure to file
                        imageCounter = imageCounter + 1
                        #print("blobConfidence=",blobConfidence,"insideTrialPoints=",insideTrialPoints)
                        blob_file = open(save_path + "file_number%d"%data_num[i]+"_contour_number_%d"%j+".txt", "w")
    ...
    if (show_anim == True):
        ...
        ionDensity= open_path  +  '%s_electron_M0_%d.bp' %(filename, data_num[0])
        phi= open_path  + '%s_phi_%d.bp' %(filename, data_num[0])
        ionDensityData = pg.data.GData(ionDensity)
        phiData = pg.data.GData(phi)
        x_vals,y_vals,X,Y,ionDensityGrid,eyGrid = func_data(ionDensityData,phiData)
        cp1 = ax1.contourf(X, Y, ionDensityGrid, cnum, cmap=color)
        ...
        if(save_anim == True):
            ...
            ani.save(save_path + filename + '_raytrace_animation.gif')
    pbar.close()
